...
class ST_Attn(object):
    ...
    def _encoderlayer(self, x, idx):
        ...
        with tf.variable_scope('encoderlayer'):
            ...
            state_t_ = self._temporalAttn(x_t, x_t, x_t, reuse=(idx!=0))

            # exchange S-T
            # state_s_, state_t_ = self._exchange_ST(state_s, state_t, coder='encoder', idx=idx)

            # feedforward
            state_t_ = self._positionwisefeedforward(state_t_, idx=idx)
    ...
    def _decoder(self, state):
        with tf.variable_scope('decoder'):
            ...
            y = tf.concat([self.yh, self.yz], axis=-1)
            # y = tf.add(self.yh, self.yz)

            x_shape = y.get_shape()

            # y_s = tf.reshape(tf.transpose(y, [0,2,3,1,4]),
            #                  shape=[-1, x_shape[2], x_shape[3], x_shape[1] * x_shape[-1]])
            y_t = tf.reshape(y, [-1, x_shape[1], x_shape[2] * x_shape[3] * x_shape[4]])
            # change for later shape
    ...
    def _decoderlayer(self, y, state, idx):
        ...
        with tf.variable_scope('decoderlayer'):
            ...
            with tf.variable_scope('EA'):
                ...
                y_t_ = self._temporalAttn(y_t, state_t, state_t, reuse=(idx!=0))

            # exchange S-T
            # y_s_, y_t_ = self._exchange_ST(y_s, y_t, coder='decoder', idx=2*idx+1)

            # feedforward
            y_t_ = self._positionwisefeedforward(y_t_, idx=idx)
    ...
    def _positionwisefeedforward(self, x_t, idx):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('ST-Attn'):
            ...
            # encoder-decoder
            state = self._encoder(x_t)
            y_t = self._decoder(state)
