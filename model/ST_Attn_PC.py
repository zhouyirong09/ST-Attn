...
class ST_Attn(object):
    def __init__(self, input_dim=[64, 64, 2], batch_size=32, input_steps=10, output_steps=10, ext_dim=8,
                 ext_data={}, hm_data={}, pc_data=[], params={}):
    ...
    def _conv(self, dim, input, filters, kernel_size, padding='same', name=None, activator=None, bn=False, dropout=False):
        ...
        if bn == False:
            ...
            #     output = activator(tf.layers.batch_normalization(output))
            output = tf.layers.batch_normalization(output)
        return output
    ...
    def _encoderlayer(self, x, idx):
        ...
        with tf.variable_scope('encoderlayer', reuse=(idx!=0)):
            ...
            state_s = tf.layers.batch_normalization(state_s + state_s_, name='ec_res{}s'.format(idx))
            state_t = tf.layers.batch_normalization(state_t + state_t_, name='ec_res{}t'.format(idx))
    ...
    def _decoderlayer(self, y, state, idx):
        ...
        with tf.variable_scope('decoderlayer'):
            ...
            # exchange S-T
            y_s_, y_t_ = self._exchange_ST(y_s, y_t, coder='decoder', idx=idx)

            # feedforward
            y_s_, y_t_ = self._positionwisefeedforward(y_s_, y_t_, idx=idx)

            y_s = tf.layers.batch_normalization(y_s + y_s_, name='dc_res{}s'.format(idx))
            y_t = tf.layers.batch_normalization(y_t + y_t_, name='dc_res{}t'.format(idx))
            print('decoderlayer')
        return y_s, y_t
    ...
    def _pcAttn(self, state):
        state_s, state_t = state

        v = tf.convert_to_tensor(self.pc_data, dtype=tf.float32)
        v_shape = v.get_shape()

        # transform pc_data to state shape
        v_s = tf.reshape(tf.transpose(v, [0, 2, 3, 1, 4]),
                       shape=[-1, v_shape[2], v_shape[3], v_shape[1] * v_shape[-1]])
        v_t = tf.reshape(v, [-1, v_shape[1], v_shape[2]*v_shape[3]*v_shape[4]])
        v_s = self._conv(2, v_s, self.s_attn_dim,
                       kernel_size=[3,3], padding='same', name='pc_s',
                       activator='relu', bn=True, dropout=self.dropout_rate)
        v_t = self._conv(1, v_t, self.t_attn_dim,
                       kernel_size=1, padding='same', name='pc_t1',
                       activator='relu', bn=True, dropout=self.dropout_rate)
        v_t = self._conv(1, tf.transpose(v_t, [0,2,1]), self.input_steps,
                         kernel_size=1, padding='same', name='pc_t2',
                         activator='relu', bn=True, dropout=self.dropout_rate)

        # reshape for attn_map
        v_s = tf.tile(tf.expand_dims(
            tf.reshape(v_s, [-1, v_shape[2]*v_shape[3]*self.s_attn_dim]), 0),
            [tf.shape(state_s)[0], 1, 1])
        v_t = tf.tile(tf.expand_dims(
            tf.reshape(tf.transpose(v_t, [0, 2, 1]), [-1, self.input_steps*self.t_attn_dim]),0),
            [tf.shape(state_t)[0], 1, 1])

        state_s = tf.expand_dims(tf.reshape(state_s, [-1, v_shape[2]*v_shape[3]*self.s_attn_dim]), 1)
        state_t = tf.expand_dims(tf.reshape(tf.transpose(state_t, [0, 2, 1]), [-1, self.input_steps*self.t_attn_dim]), 1)

        # f, g, h
        t_attn = tf.matmul(state_t, v_t, transpose_b=True)
        s_attn = tf.matmul(state_s, v_s, transpose_b=True)
        # softmax(Q, K)*V
        t_attn = tf.nn.softmax(t_attn, axis=-1)
        s_attn = tf.nn.softmax(s_attn, axis=-1)
        # output
        state_t = tf.matmul(t_attn, v_t)
        state_s = tf.matmul(s_attn, v_s)
        state_t = tf.layers.dropout(state_t, rate=self.dropout_rate)
        state_s = tf.layers.dropout(state_s, rate=self.dropout_rate)

        # reshape
        state_s = tf.reshape(tf.squeeze(state_s), [-1, self.input_row, self.input_col, self.s_attn_dim])
        state_t = tf.reshape(tf.squeeze(state_t), [-1, self.input_steps, self.t_attn_dim])
    ...
    def build_model(self):
        x = self.x # use history mean information as position-encoding
        y = self.y
        x_ext = self.x_ext
        y_ext = self.y_ext
