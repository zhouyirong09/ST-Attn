...
class ST_Attn(object):
    ...
    def _decoder(self, state, y_ext):
        with tf.variable_scope('decoder'):
            ...
            y_s = tf.reshape(tf.transpose(y_ext, [0,2,3,1,4]),
                             shape=[-1, x_shape[2], x_shape[3], x_shape[1] * x_shape[-1]])
            y_t = tf.reshape(y_ext, [-1, x_shape[1], x_shape[2] * x_shape[3] * x_shape[4]])
            # change for later shape
    ...
    def _pcAttn(self, state):
        ...
        v_k = self._conv(2, tf.reshape(tf.transpose(v, [0,2,3,1,4]), [-1,v_shape[2],v_shape[3],v_shape[1]*v_shape[4]]), v_shape[1]*v_shape[4],
                         kernel_size=[3,3], padding='same',
                         activator='relu', bn=True)

        v_v = self._conv(2, tf.reshape(tf.transpose(v, [0,2,3,1,4]), [-1,v_shape[2],v_shape[3],v_shape[1]*v_shape[4]]), v_shape[1]*v_shape[4],
                         kernel_size=[3,3], padding='same',
                         activator='relu', bn=True)
        v_k = tf.reshape(v_k, shape=[v_shape[0], -1])
        v_k = tf.tile(tf.expand_dims(v_k, 0), [tf.shape(state_s)[0], 1, 1])
        v_v = tf.reshape(v_v, shape=[v_shape[0], -1])
        v_v = tf.tile(tf.expand_dims(v_v, 0), [tf.shape(state_s)[0], 1, 1])

        # transform state to v shape
        state_t = self._conv(1, state_t, v_shape[2]*v_shape[3],
                             kernel_size=1, padding='same',
                             activator='relu', bn=True)
        state_t = tf.reshape(state_t, shape=[-1, self.input_steps, v_shape[2], v_shape[3]])
        state_t = tf.transpose(state_t, [0, 2, 3, 1])
        state_ = tf.concat([state_s, state_t], axis=-1)
        state_ = self._conv(2, state_, self.output_steps * v_shape[-1],
                            kernel_size=1, padding='same',
                            activator='relu', bn=True)
        state_ = tf.reshape(state_, shape=[-1, v_shape[2], v_shape[3], self.output_steps, v_shape[-1]])
        state_ = tf.transpose(state_, [0, 3, 1, 2, 4])
        state_ = tf.reshape(state_, shape=[-1, v_shape[1]*v_shape[2]*v_shape[3]*v_shape[4]])
        state_ = tf.expand_dims(state_, 1)

        # f, g, h
        attn = tf.matmul(state_, v_k, transpose_b=True)

        # softmax(Q, K)*V
        attn = tf.nn.softmax(attn, axis=-1)

        # output
        state_ = tf.matmul(attn, v_v)

        # reshape
        state_ = tf.reshape(tf.squeeze(state_), [-1, self.input_row, self.input_col, self.output_steps, self.input_channel])
        state_ = tf.transpose(state_, [0,3,1,2,4])
